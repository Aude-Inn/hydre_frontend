...
export function AdminDashboardMessage() {
  ...
  const handleReply = (msg: MessageData) => {
    const text = prompt(`Répondre à ${msg.userName || `#${msg.userId}`}`, "");
    if (!text?.trim()) return;
            ...
            messages.map((msg) => (
              <tr key={msg._id} className="border-b hover:bg-white/5">
                ...
                <td className="py-2 px-4 flex gap-2">
                  <button onClick={() => handleReply(msg)} className="bg-blue-500 text-white rounded-full w-8 h-8">
                    💬
                  </button>
                  <button onClick={() => handleDelete(msg._id)} className="bg-red-500 text-white rounded-full w-8 h-8">
                    ❌
                  </button>
                </td>
              </tr>
